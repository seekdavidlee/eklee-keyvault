...
The repository must enforce this promotion flow with the required
`release/promotion-policy` pull request check: only a same-repository `release/*`
branch may target `main`. The check prevents direct development-branch merges
from deploying to the production environment and from bypassing the documented
cleanup lifecycle. Branch rules remain a GitHub repository configuration
concern, so this check must be required by the `main` ruleset.
